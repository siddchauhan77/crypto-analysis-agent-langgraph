...
- The fixed dataset contains 20 cases across the six planned groups and runs each case in an isolated thread.
- The initial two-repetition run produced 40 runs. Thirty-seven passed every deterministic rule.
- Two threaded news runs omitted the provider label. One synthesis run omitted an explicit causal caveat.
- The upgrade moved provider, retrieval-time, and causal-uncertainty invariants to deterministic graph-boundary enforcement and added regression tests.
- The unchanged 40-run set then passed 40 of 40. Every deterministic check scored 100%.
- Regression-run median latency was 4.835 seconds. Estimated GPT-4o mini cost was $0.022025.
- The evaluation exposed and corrected an invalid historical NewsAPI date argument generated by the model.
- The initial three response-contract failures remain documented as the regression history.
- Sixty-two offline tests pass. Two client-level live tests remain opt-in.
- Deterministic results, manual review, methodology, and limitations are separated in `docs/evaluation-report.md`.
...
- Sixty-two offline tests pass. Two provider tests remain opt-in.

### Phase 8. Final QA and portfolio packaging, 1 hour

Status: Completed August 13, 2026 for the technical MVP and regression upgrade. Lint, 62 offline tests, two skipped opt-in live tests, a 40-of-40 fixed-suite evaluation, credential-pattern scan, live HTTP checks, desktop workflow, mobile overflow, and browser console checks passed.
...
The measured evidence now exists. Convert it into distribution: a 90-second demo, a clear portfolio case study, and direct outreach to five technical reviewers.
