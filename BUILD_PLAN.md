...
- Two live repetitions produced 40 runs. Thirty-seven passed every deterministic rule for a 92.5% baseline.
- Acceptable first tool, required tools, symbol arguments, provider success, exact retrieval time, and trade safety scored 100%.
- Median end-to-end latency was 5.574 seconds. Estimated GPT-4o mini cost was $0.021383.
- The evaluation exposed and corrected an invalid historical NewsAPI date argument generated by the model.
- Three response-contract failures remain published: two missing provider labels on a threaded follow-up and one missing explicit causal-uncertainty sentence.
- Fifty-five offline tests pass. Two client-level live tests remain opt-in.
